...
async def test_create_and_remove_worktree_lifecycle() -> None:
    name = f"test-{uuid.uuid4().hex[:8]}"
    branch = f"autofix-test/{name}"

    path = await create_worktree(name, branch)
    try:
        assert path.is_dir()
...
async def test_reset_worktree_discards_uncommitted_changes() -> None:
    name = f"test-{uuid.uuid4().hex[:8]}"
    branch = f"autofix-test/{name}"
    path = await create_worktree(name, branch)
    try:
